...
| `whittaker/plot_whittaker_solvers.py` | `ruviz_pybaselines_gallery_whittaker_solver_timings` | Uses the same `_make_data` default signal, lambda equation, data sizes `[499, 935, 1748, 3270, 6115, 11437, 21388, 40000]`, `repeats=25`, AsLS, `max_iter=8`, and `tol=-1` to force fixed iterations. The Rust-native timing comparison is allocating `asls` vs reusable-workspace `asls_into`, since this crate has one native pentadiagonal solver instead of SciPy/pentapy backend selection. |
