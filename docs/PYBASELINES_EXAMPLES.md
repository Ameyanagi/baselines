...
| `whittaker/plot_lam_vs_data_size.py` | pending | Heavy parameter sweep; not yet ported. |
...
| `spline/plot_lam_vs_num_knots.py` | pending | Heavy parameter sweep; Rust `mixture_model` does not yet expose `num_knots`. |
