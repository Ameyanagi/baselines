...
pub fn snip_batch_into(
    ...
    {
        input
            .par_chunks_exact(n_points)
            .zip(output.par_chunks_exact_mut(n_points))
            .map(|(spectrum, baseline)| snip_into(spectrum, params, baseline))
            .collect()
    }
...
fn validate_batch(input: &[f64], n_spectra: usize, n_points: usize, output: &[f64]) -> Result<()> {
    ...
    let expected = n_spectra
        ...
        .ok_or(BaselineError::InvalidParameter {
            name: "batch shape",
            reason: "n_spectra * n_points overflows usize",
        })?;
