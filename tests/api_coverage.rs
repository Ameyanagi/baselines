...
use baselines::optimizers::{
    ...
    custom_bc, custom_bc_with, optimize_extended_range,
};
...
fn exposed_1d_algorithms_return_finite_baselines() {
    ...
    let mut fits: Vec<Fit> = vec![
        ...
        jbcd(&y, morph).unwrap(),
        mixture_model(&y, MixtureModelParams::default()).unwrap(),
        ...
        dietrich(&y, DietrichParams::default()).unwrap(),
        ...
        custom_bc(&y, CustomBcParams::default()).unwrap(),
        custom_bc_with(&y, CustomBcParams::default(), |values| {
            asls(values, AslsParams::default())
        })
        .unwrap(),
        interp_pts(
            &y,
